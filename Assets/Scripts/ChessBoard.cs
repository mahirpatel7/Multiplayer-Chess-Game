...
public class ChessBoard : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(ray, out info, 100, LayerMask.GetMask("Tile", "Hover", "Highlight")))
        {
            ...
            if(Input.GetMouseButtonDown(0)){
                if(chessPieces[hitPosition.x, hitPosition.y] != null)
                {
                    ...
                    if((chessPieces[hitPosition.x, hitPosition.y].team == 0 && isWhiteTurn && currentTeam == 0) || (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !isWhiteTurn && currentTeam == 1))
                    {
                        ...
                        // Get a list of where can i go, highlight tiles as well
            ...
            if(currentlyDragging != null && Input.GetMouseButtonUp(0))
            {
                ...
                Vector2Int previousPosition = new Vector2Int(currentlyDragging.cuurentX, currentlyDragging.cuurentY);
        ...
        else
        {
            ...
            {
                currentlyDragging.SetPosition(GetTileCenter(currentlyDragging.cuurentX, currentlyDragging.cuurentY));
                currentlyDragging = null;
                RemoveHighlightTiles();
            }
        }
    ...
    {
        chessPieces[x, y].cuurentX = x;
        chessPieces[x, y].cuurentY = y;
    ...
    private void ProcessSpecialMove()
    {
        if(specialMove == SpecialMove.EnPassant)
        {
            ...
            if(myPawn.cuurentX == enemyPawn.cuurentX)
            {
                if(myPawn.cuurentY == enemyPawn.cuurentY - 1 || myPawn.cuurentY == enemyPawn.cuurentY + 1)
                {
                    if(enemyPawn.team == 0)
                    {
                        deadWhites.Add(enemyPawn);
                        enemyPawn.SetScale(Vector3.one * 160 *deathSize);
                        enemyPawn.SetPosition(
                            new Vector3(8 * tileSize, yOffset, -1 * tileSize)
                            - bounds
                            + new Vector3(tileSize / 2, 0, tileSize / 2)
                            + (Vector3.forward * deathSpacing * 15/2) * deadWhites.Count);
                    }
                    else
                    {
                        ...
                        enemyPawn.SetPosition(
                            ...
                            - bounds
                            + new Vector3(tileSize / 2, 0, tileSize / 2)
                            + (Vector3.back * deathSpacing * 15/2) * deadBlacks.Count);
                    }
                    chessPieces[enemyPawn.cuurentX, enemyPawn.cuurentY] = null;
                }
            }
        }
    ...
    private void PreventCheck()
    {
        ...
        // Since we're are sending ref availableMoves, we will be deleting moves that are putting us in check
    ...
    private void SimulateMoveForSinglePiece(ChessPiece cp, ref List<Vector2Int> moves, ChessPiece targetKing)
    {
        ...
        int actualX = cp.cuurentX;
        int actualY = cp.cuurentY;
        ...
        for(int i = 0; i < moves.Count; i++)
        {
            ...
            Vector2Int kingPositionThisSim = new Vector2Int(targetKing.cuurentX, targetKing.cuurentY);

            // Did we simulate the king's move
            if(cp.type == ChessPieceType.King)
                kingPositionThisSim =new Vector2Int(simX, simY);

            // copy the [,] and not a referance
            ...
            var deadPiece = simAttackingPieces.Find(c => c.cuurentX == simX && c.cuurentY == simY);
            if(deadPiece != null)
                simAttackingPieces.Remove(deadPiece);

            // Get all the simulated attacking pieces
    ...
    private bool CheckForCheckmate()
    {
        ...
        if(ContainsValidMove(ref currentAvailableMoves, new Vector2Int(targetKing.cuurentX, targetKing.cuurentY)))
        {
    ...
    private void MoveTo(int originalX, int originalY, int x, int y)
    {
        ...
        if(chessPieces[x, y] != null)
        {
            ...
            if(ocp.team == 0)
            {
                ...
                deadWhites.Add(ocp);
                ocp.SetScale(Vector3.one * 160 *deathSize);
                ocp.SetPosition(
                    new Vector3(8 * tileSize, yOffset, -1 * tileSize)
                    - bounds
                    + new Vector3(tileSize / 2, 0, tileSize / 2)
